    ...
    while True:
        ...
        if data.get("connected"):
            ...
            if (
                ...
                print("Recon connected with valid global position.")
                return data
...
def wait_until_armed(telemetry, timeout=120):
...
try:
    ...
    arm_ack = telemetry.wait_command_ack(
        mavutil.mavlink.MAV_CMD_COMPONENT_ARM_DISARM,
        timeout=20
    )

    if arm_ack is not None:
        print("ARM ACK result:", arm_ack.result)
    else:
        print("ARM ACK: no ACK received")
    ...
    takeoff_ack = telemetry.wait_command_ack(
        mavutil.mavlink.MAV_CMD_NAV_TAKEOFF,
        timeout=20
    )

    if takeoff_ack is not None:
        print("TAKEOFF ACK result:", takeoff_ack.result)
    else:
        print("TAKEOFF ACK: no ACK received")
