...
try:
    ...
    print("Warning: Gazebo camera Python bindings unavailable:", e)
...
# File Paths
CAMERA_PARAMS_FILE = "/home/maha/Desktop/project/camera_params.json"
EVENT_FILE = "/home/maha/Desktop/project/latest_target_event.json"
DETECTION_IMAGE = "/home/maha/Desktop/project/latest_detection.jpg"

# Caffe model paths (MobileNet SSD)
CAFFE_PROTO = "/home/maha/Desktop/project/person_detection_model/deploy.prototxt"
CAFFE_WEIGHTS = "/home/maha/Desktop/project/person_detection_model/mobilenet_iter_73000.caffemodel"
...
# Runtime mode:
#   default  -> real UAV camera + serial MAVLink
#   --sitl   -> Gazebo Recon camera + SITL MAVLink over UDP
SITL_MODE = "--sitl" in sys.argv
GAZEBO_RECON_CAMERA_TOPIC = (
    "/world/bordershield_world/model/recon_drone/"
    "model/gimbal/link/pitch_link/sensor/camera/image"
)
SITL_MAVLINK_CONNECTION = "udpin:0.0.0.0:14552"
...
if hasattr(cv2.dnn, "readNetFromCaffe"):
    ...
    print("OpenCV 5 detected: loading Caffe model using cv2.dnn.readNet")
    old_net = cv2.dnn.readNet(CAFFE_WEIGHTS, CAFFE_PROTO, "caffe")

print("Loading Darknet YOLOv3-Tiny Drone model...")

DRONE_MODEL_AVAILABLE = False
DRONE_CLASSES = []
drone_net_gpu = None
drone_net = None
drone_layers = []
...
drone_files_ok = (
    os.path.exists(DRONE_CFG)
    and os.path.exists(DRONE_WEIGHTS)
    and os.path.exists(DRONE_NAMES)
)

if not drone_files_ok:
    print("DRONE AI DISABLED: custom YOLO drone model files are not available on this SITL machine.")
    print("PERSON / CAR / BUS / MOTORBIKE detection will continue normally.")
else:
    ...
    if DARKNET_GPU_AVAILABLE:
        try:
            ...
            DRONE_MODEL_AVAILABLE = True
            print("Successfully loaded Darknet GPU model.")

        except Exception as e:
            print("Failed to initialize Darknet GPU model:", e)
            DARKNET_GPU_AVAILABLE = False

    if not DARKNET_GPU_AVAILABLE:
        try:
            print("Loading drone model via OpenCV CPU fallback...")

            drone_net = cv2.dnn.readNetFromDarknet(
                DRONE_CFG,
                DRONE_WEIGHTS
            )
            ...
            drone_net.setPreferableBackend(
                cv2.dnn.DNN_BACKEND_DEFAULT
            )
            drone_net.setPreferableTarget(
                cv2.dnn.DNN_TARGET_CPU
            )

            DRONE_MODEL_AVAILABLE = True
            print("Drone model loaded using OpenCV CPU.")

        except Exception as e:
            print("DRONE AI DISABLED:", e)
            DRONE_MODEL_AVAILABLE = False
...
class GazeboCamera:
    """OpenCV-like camera wrapper for the Gazebo Recon image topic."""

    def __init__(self, topic):
        if not GAZEBO_CAMERA_AVAILABLE:
            raise RuntimeError(
                "Gazebo camera bindings are unavailable. "
                "Expected gz.transport13 and gz.msgs10."
            )
        ...
        ok = self.node.subscribe(Image, self.topic, self._callback)
        if not ok:
            raise RuntimeError(
                "Could not subscribe to Gazebo camera topic: {}".format(self.topic)
            )

        print("GAZEBO CAMERA SUBSCRIBED: {}".format(self.topic))
    ...
    def _callback(self, msg):
        try:
            ...
            if data.size != expected_rgb:
                if self._received_frames < 3:
                    print(
                        "Gazebo camera frame skipped: expected {} bytes, got {}".format(
                            expected_rgb, data.size
                        )
                    )
                return
            ...
            print("Gazebo camera frame error:", e)
    ...
    def release(self):
        # Gazebo transport Node lifetime owns the subscription.
        pass
...
# ----------------------------------------------------
if SITL_MODE:
    print("CAMERA MODE: GAZEBO SITL")
    try:
        cap = GazeboCamera(GAZEBO_RECON_CAMERA_TOPIC)
        selected_camera_device = GAZEBO_RECON_CAMERA_TOPIC
    except Exception as e:
        print("CRITICAL ERROR: Could not initialize Gazebo camera: {}".format(e))
        os._exit(1)

    print("Waiting for first Gazebo camera frame...")
    first_frame_deadline = time.time() + 15.0
    while time.time() < first_frame_deadline:
        ok, test_frame = cap.read()
        if ok and test_frame is not None:
            print("GAZEBO CAMERA READY")
            print("Camera frame shape: {}".format(test_frame.shape))
            break
        time.sleep(0.1)
    else:
        print("CRITICAL ERROR: Timed out waiting for Gazebo camera frames.")
        os._exit(1)
else:
    print("CAMERA MODE: REAL UAV")
...
if SITL_MODE:
    print("TELEMETRY MODE: SITL {}".format(SITL_MAVLINK_CONNECTION))
    telemetry_service = MAVLinkTelemetry(
        connection_string=SITL_MAVLINK_CONNECTION
    )
else:
    print("TELEMETRY MODE: REAL UAV /dev/ttyACM0 @ 115200")
    telemetry_service = MAVLinkTelemetry(
...
try:
    while True:
        ...
        if DRONE_MODEL_AVAILABLE and DARKNET_GPU_AVAILABLE:
